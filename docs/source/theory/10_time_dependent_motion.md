...
nothing about *when* the body is at a given angle. This chapter closes that
gap in two complementary ways: an exact analytic route through an auxiliary
...
$\varepsilon$ (chapter 7) recover $(a,e)$, the orbital plane, and the initial
anomaly, closing the loop between the two representations.
